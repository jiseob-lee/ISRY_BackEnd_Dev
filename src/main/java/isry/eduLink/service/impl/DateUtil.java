...
public class DateUtil {
    ...
    * @return 변환된 문자열
     ...
     * @return 범위 리스트
     */
    public static String[] getDateArrByTermCfx(String sdateVal, String edate, int term, String res_form) {
    	String sdate = sdateVal;
    	if (sdate.length() < 8 || edate.length() < 8) {
			return new String[]{};
    	}

		while (sdate.length() < 14) {
			sdate += "0";
		}
		...
		while(true)
		{
			date = new SimpleDateFormat("yyyyMMddHHmmss", Locale.KOREAN).format(cal.getTime());
			list.add(new SimpleDateFormat(res_form, Locale.KOREAN).format(cal.getTime()));

			if (date.compareTo(edate) > 0) {
				break;
			}

			cal.add(Calendar.SECOND, term);
		}

		if(list.size() > 0) list.remove(list.size()-1);

		String[] str_arr = new String[list.size()];
		list.toArray(str_arr);
		return str_arr;
	}
}
