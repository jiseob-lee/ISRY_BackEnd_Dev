 ...
 ******************************************************************************************/
package isry.eduLink.service.impl;
 ...
 * -                
 */
@Service("eduLinkService")
public class EduLinkServiceImpl implements EduLinkService {

	@Resource(name="eduLinkMapper")
    public EduLinkMapper eduLinkMapper;
	
	@Resource(name="renuNoMapper")
    private RenuNoMapper renuNoMapper; 
	
	private static final Logger LOGGER = LoggerFactory.getLogger(EduLinkServiceImpl.class);
	 ...
	 * @Method설명 : 교육부 연계 저장  
	 */
	public void eduBatch() throws Exception {

		List<Map<String, String>> CAD100List = eduLinkMapper.selectCad100();
...
//		String sStdntEml		= "";   // 학생이메일(암호화)
		...
		String sSggCd			= ""; // 시군구코드	
		String sEnfsnNo 		= "";
		//LOGGER.debug("dsExcel ::::::::::: " + CAD100List.size());
		List<String> moeSchulmCodeList = eduLinkMapper.moeSchulmCodeList();
		
		// 현재시간 10자리
		LocalDateTime nowTime = LocalDateTime.now(); // YYYY-MM-DDTHH:MI:SS.MIS
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyMMddHHmm");
		String sTime = nowTime.format(formatter);
		Map<String, String> grpMap = new HashMap<String, String>();
		for(int i=0; i<CAD100List.size(); i++) {
			grpMap = CAD100List.get(i);
			String esbSeq = String.valueOf(grpMap.get("ESB_SEQ"));
			sUldRqstdoSttsSeCd = "01";
			sErrInfoDtlCn = "";
			String moeCode = "";
			if(!"".equals(grpMap.get("STDNT_MBL_TELNO_ENCPT")) && null != grpMap.get("STDNT_MBL_TELNO_ENCPT")) {
				iMblTelnoErr = grpMap.get("STDNT_MBL_TELNO_ENCPT").length(); // 학생핸드폰번호 갯수
			}
			
			
			if("01".equals(grpMap.get("STDNT_WRTCNS_SBMSN_YN"))) {
				grpMap.put("STDNT_WRTCNS_SBMSN_YN",  "N"); 					// 학생동의서제출여부	
			} else {
				grpMap.put("STDNT_WRTCNS_SBMSN_YN",  "Y"); 					// 학생동의서제출여부
			}
			if("01".equals(grpMap.get("STTY_AGT_WRTCNS_SBMSN_YN"))) {
				grpMap.put("STTY_AGT_WRTCNS_SBMSN_YN",  "N"); 				// 법정대리인동의서제출여부	
			} else {
				grpMap.put("STTY_AGT_WRTCNS_SBMSN_YN",  "Y"); 				// 법정대리인동의서제출여부	
			}	
			
			if(grpMap.get("STDNT_FLNM_ENCPT") == null && "".equals(grpMap.get("STDNT_FLNM_ENCPT")) && "null".equals(grpMap.get("STDNT_FLNM_ENCPT"))) {
				sErrInfoDtlCn += " / 학생 성명이 누락되었습니다.";
			} else {
				//grpMap.put("STDNT_FLNM_ENCPT",  scpDb.scpEncB64( grpMap.get("STDNT_FLNM_ENCPT"))); // 학생성명 암호화
			}
			if(grpMap.get("STDNT_BRTH_YMD") == null || "".equals(grpMap.get("STDNT_BRTH_YMD")) || "null".equals(grpMap.get("STDNT_BRTH_YMD"))) {
				sErrInfoDtlCn += " / 생년월일이 없습니다.";
			} else {
				grpMap.put("STDNT_BRTH_YMD",  grpMap.get("STDNT_BRTH_YMD").replace(".", ""));	// 학생출생일자	
				sStdntBrth		= grpMap.get("STDNT_BRTH_YMD");   // 학생생년월일
				// 학생나이 구하기
				sStdntAge = eduLinkMapper.selectStdntAge(grpMap.get("STDNT_BRTH_YMD").replace(".", ""));
				iStdntAge = Integer.valueOf(sStdntAge);
			}
				
			if(grpMap.get("STDNT_ADDR") == null && "".equals(grpMap.get("STDNT_ADDR")) && "null".equals(grpMap.get("STDNT_ADDR"))) {
				sErrInfoDtlCn += " / 주소가 누락되었습니다.";
			}
			
			if(grpMap.get("STDNT_MBL_TELNO_ENCPT") == null && "".equals(grpMap.get("STDNT_MBL_TELNO_ENCPT")) && "null".equals(grpMap.get("STDNT_MBL_TELNO_ENCPT"))) {
				sErrInfoDtlCn += " / 연락처가 필요합니다.";
			}
			
			subMap03.put("STDNT_FLNM_ENCPT", sStdntNm);
			subMap03.put("STDNT_BRTH_YMD", sStdntBrth);
			subMap03.put("STDNT_SXDC_NM", grpMap.get("STDNT_SXDC_NM"));
			subMap03.put("STDNT_TELNO", grpMap.get("STDNT_TELNO"));
			subMap03.put("STDNT_MBL_TELNO_ENCPT", sStdntMblTelno);
			sDupChk = eduLinkMapper.selectDupChk(subMap03);
			
			// 우선순위2_특별관리
			sSchulwCn = grpMap.get("SCHULW_DSCNTC_CS_CN");
			
			// 우선순위1_오류정보
			sMblTelnoErr = String.valueOf(iMblTelnoErr); // 학생전화번호번호 갯수
			
			if(!"".equals(grpMap.get("STDNT_TELNO")) && null != grpMap.get("STDNT_TELNO")) {
				iTelnoErr = grpMap.get("STDNT_TELNO").length(); // 학생전화번호번호 갯수
			}
			
			sTelnoErr = String.valueOf(iTelnoErr); // 학생전화번호번호 갯수
			
			sStdntAddr = grpMap.get("STDNT_ADDR"); // 학생주소
			
			boolean bSpclaMngYn = false;
			if(!"".equals(sSchulwCn) && null != sSchulwCn) {
				...
				sUldRqstdoSttsSeCd = "03";
				sErrInfoDtlCn += " / 학업중단 사유가 없습니다.";
			}
			...
			String sLastSchlGradeNm = grpMap.get("LAST_SCHL_GRADE_NM");
			...
			//String oCtpvNM[] = grpMap.get("STDNT_ADDR").split("\n");
			if(!"".equals(grpMap.get("STDNT_ADDR")) && null != grpMap.get("STDNT_ADDR")) {
				String oSggNm[] = grpMap.get("STDNT_ADDR").split(" ");  
			...
			// 주소 유효성 체크 끝
			LOGGER.debug("sUldRqstdoSttsSeCd ::::::::::: " + sUldRqstdoSttsSeCd);
			LOGGER.debug("sErrInfoDtlCn ::::::::::: " + sErrInfoDtlCn);
			
			List<Map<String, String>> MoeList = eduLinkMapper.selectSac100("MOE_OFCDC_SE_CD");
			moeCode = grpMap.get("MOE_OFCDC_SE_CD");
			if("".equals(moeCode) || null == moeCode) {
				moeCode = "A00";
			}
			String sEnfsnInstNo = "";
			String sUserId = "BATCH"; 
			
			for(Map<String, String> map : MoeList) {
				if(map.get("CMMNS_CD_VALUE").equals(moeCode)) {
					sEnfsnInstNo = map.get("ADDTNG_MNG_VALUE1");
					//sUserId = map.get("CMMNS_CD_VALUE_NM");
				}
			}
			
			//if("01".equals(sUldRqstdoSttsSeCd) || "02".equals(sUldRqstdoSttsSeCd)) { // 01:신규등록 02:중복등록
			if("01".equals(sUldRqstdoSttsSeCd)) { // 01:신규등록. 중복 등록 오류로 뺀다 
				...
				srvcPvsnMap.put("CLINT_NO", moeCode);								// 의뢰자번호_(교육청담당자 종사자번호)				
				srvcPvsnMap.put("RQST_INST_NO", sEnfsnInstNo);						// 의뢰기관번호_(소속교육청기관번호 종사자 기관번호)
				srvcPvsnMap.put("RQST_CS_CN", "교육청 연계");						// 의뢰사유내용
				...
				String oSggNm[] = grpMap.get("STDNT_ADDR").split(" ");  
				String sCtpvSeCd = "";
				if(oSggNm[0].indexOf("강원") >= 0) {
					oSggNm[0] = "강원도";
					sCtpvSeCd = "42";
				}
				if(oSggNm[0].indexOf("경기") >= 0) {
					oSggNm[0] = "경기도";
					sCtpvSeCd = "41";
				}
				if(oSggNm[0].indexOf("경남") >= 0) {
					oSggNm[0] = "경상남도";
					sCtpvSeCd = "48";
				}
				if(oSggNm[0].indexOf("경북") >= 0) {
					oSggNm[0] = "경상북도";
					sCtpvSeCd = "47";
				}
				if(oSggNm[0].indexOf("광주") >= 0) {
					oSggNm[0] = "광주광역시";
					sCtpvSeCd = "29";
				}
				if(oSggNm[0].indexOf("대구") >= 0) {
					oSggNm[0] = "대구광역시";
					sCtpvSeCd = "27";
				}
				if(oSggNm[0].indexOf("대전") >= 0) {
					oSggNm[0] = "대전광역시";
					sCtpvSeCd = "30";
				}
				if(oSggNm[0].indexOf("부산") >= 0) {
					oSggNm[0] = "부산광역시";
					sCtpvSeCd = "26";
				}
				if(oSggNm[0].indexOf("서울") >= 0) {
					oSggNm[0] = "서울특별시";
					sCtpvSeCd = "11";
				}
				if(oSggNm[0].indexOf("세종") >= 0) {
					oSggNm[0] = "세종특별자치시";
					sCtpvSeCd = "36";
				}
				if(oSggNm[0].indexOf("울산") >= 0) {
					oSggNm[0] = "울산광역시";
					sCtpvSeCd = "31";
				}
				if(oSggNm[0].indexOf("인천") >= 0) {
					oSggNm[0] = "인천광역시";
					sCtpvSeCd = "28";
				}
				if(oSggNm[0].indexOf("전남") >= 0) {
					oSggNm[0] = "전라남도";
					sCtpvSeCd = "46";
				}
				if(oSggNm[0].indexOf("전북") >= 0) {
					oSggNm[0] = "전라북도";
					sCtpvSeCd = "45";
				}
				if(oSggNm[0].indexOf("제주") >= 0) {
					oSggNm[0] = "제주특별자치도";
					sCtpvSeCd = "50";
				}
				if(oSggNm[0].indexOf("충남") >= 0) {
					oSggNm[0] = "충청남도";
					sCtpvSeCd = "44";
				}
				if(oSggNm[0].indexOf("충북") >= 0) {
					oSggNm[0] = "충청북도";
					sCtpvSeCd = "43";
				}
				LOGGER.debug("oSggNm[0]	 @@@@@@@@"          + oSggNm[0]);
				LOGGER.debug("oSggNm[1]	 @@@@@@@@"          + oSggNm[1]);
				
				subMap04.put("CTPV_NM", oSggNm[0]); 
				subMap04.put("SGG_NM", oSggNm[1]);
				subMap04.put("SGG_NM1", oSggNm[1] + " " + oSggNm[2]);
				subMap04.put("CTPV_SE_CD", sCtpvSeCd);
				
				
				// 기관번호,담당자종사자번호 구하기
				List<Map<String, Object>> list = eduLinkMapper.selectInstEnfsnNo(subMap04);
				if(list.size() > 0) {
				...
				String rcptInstNo = "0";	// 접수기관번호
				String rcptEnfsnNo = "0";	// 접수기관담당자
				
				if("".equals(subResultMap04.get("INST_NO")) || null == subResultMap04.get("INST_NO")) {
					sErrInfoDtlCn += " / 주소로 지역을 찾을 수 없습니다.";
				} else {
					rcptInstNo = subResultMap04.get("INST_NO").toString();
				}
				
				if(!"".equals(subResultMap04.get("PIC_ENFSN_NO")) && null != subResultMap04.get("PIC_ENFSN_NO")) {
					rcptEnfsnNo = subResultMap04.get("PIC_ENFSN_NO").toString();
				} 
				
				if(subResultMap04 != null) {
					sSggCd = subResultMap04.get("SGG_CD").toString(); // 시군구코드
					sEnfsnNo = subResultMap04.get("PIC_ENFSN_NO").toString();
					srvcPvsnMap.put("CLR_NO", sEnfsnNo);	// 접수자번호
				...
				// 엑셀업로드 추가정보 내용
				String lastSchlGradeNm  = grpMap.get("LAST_SCHL_GRADE_NM");						// 최종학교학년명
				String schulwDscntcYmd  = grpMap.get("SCHULW_DSCNTC_YMD").replace(".", "-");	// 학업중단일자
				String schulwDscntcCsCn = grpMap.get("SCHULW_DSCNTC_CS_CN");					// 학업중단사유내용
				String hncfrtHpeCoseNm  = grpMap.get("HNCFRT_HPE_COSE_NM");						// 향후희망진로명
...
//				srvcPvsnMap.put("FRST_RQST_NO", sSrvcPvsnRqstNo);					// 최초의뢰번호_컬럼삭제됨
				...
				srvcPvsnMap.put("FRST_RGTR_ID", "BATCH");
				srvcPvsnMap.put("LAST_MDFR_ID", "BATCH");
				srvcPvsnMap.put("LINK_SN", esbSeq);
				regCntSEA400 =eduLinkMapper.insertSrvcPvsnRqst(srvcPvsnMap); // SEB400(서비스제공의뢰) insert
				
				if (regCntSEA400 > 0) {
					// 데이터변경구분코드
					srvcPvsnMap.put("DATAA_CHG_SE_CD", "I");					
					eduLinkMapper.insertSrvcPvsnRqstHstr(srvcPvsnMap); // SEB401(서비스제공의뢰이력) insert			
				}
				...
				srvcPvsnRcptMap.put("FRST_RGTR_ID", "BATCH");
				srvcPvsnRcptMap.put("LAST_MDFR_ID", "BATCH");
				
				regCntSEA420 =eduLinkMapper.insertSrvcPvsnRcpt(srvcPvsnRcptMap); // SEB420(서비스제공접수) insert
				
				if (regCntSEA420 > 0) {
					// 데이터변경구분코드
					srvcPvsnRcptMap.put("DATAA_CHG_SE_CD", "I");					
					eduLinkMapper.insertSrvcPvsnRcptHstr(srvcPvsnRcptMap); // SEB421(서비스제공접수이력) insert			
				}
				...
				LOGGER.debug("대상자명암호화	 ::::::::" + grpMap.get("STDNT_FLNM_ENCPT"));
//				regCntSEA200Map.put("TRPR_NM_ENCPT", mask.addressMasking( scpDb.scpEncB64( grpMap.get("STDNT_FLNM_ENCPT")))); // 대상자명암호화
				regCntSEA200Map.put("TRPR_NM_ENCPT", grpMap.get("STDNT_FLNM_ENCPT")); // 대상자명암호화
				regCntSEA200Map.put("TRPR_BRTH_YMD", grpMap.get("STDNT_BRTH_YMD").replace(".", "")); // 대상자출생일자
				
				/*
				if("남성".equals(grpMap.get("STDNT_SXDC_NM"))) {
					regCntSEA200Map.put("SXDC_SE_CD", "M");							// 성별구분코드	
				}else if("여성".equals(grpMap.get("STDNT_SXDC_NM"))) {
					regCntSEA200Map.put("SXDC_SE_CD", "F");							// 성별구분코드
				}else {
					regCntSEA200Map.put("SXDC_SE_CD", "X");							// 성별구분코드
				}
				*/
				regCntSEA200Map.put("SXDC_SE_CD", grpMap.get("STDNT_SXDC_NM"));
				
				regCntSEA200Map.put("MRG_YN", "N");									// 결혼여부
				regCntSEA200Map.put("TRPR_USER_ID", "");							// 대상자사용자아이디
				regCntSEA200Map.put("TRPR_TELNO", grpMap.get("STDNT_TELNO")); // 대상자전화번호
				
//				regCntSEA200Map.put("MBL_TELNO_ENCPT", mask.addressMasking( scpDb.scpEncB64( grpMap.get("STDNT_MBL_TELNO_ENCPT")))); // 휴대전화번호암호화
//				regCntSEA200Map.put("EML_ADDR_ENCPT", mask.addressMasking( scpDb.scpEncB64( grpMap.get("STDNT_EML_ADDR_ENCPT")))); // 이메일주소암호화
				regCntSEA200Map.put("MBL_TELNO_ENCPT", grpMap.get("STDNT_MBL_TELNO_ENCPT")); // 휴대전화번호암호화
				regCntSEA200Map.put("EML_ADDR_ENCPT", grpMap.get("STDNT_EML_ADDR_ENCPT")); // 이메일주소암호화
				
				regCntSEA200Map.put("SNS_SE_CD", "");								// SNS구분코드
				regCntSEA200Map.put("MSNGR_ID_ENCPT", "");							// 메신저아이디암호화
				
				String addrZip = ""; // 우편번호
				if(!"".equals(grpMap.get("ADDR_ZIP")) && null != grpMap.get("ADDR_ZIP")) {
					addrZip = grpMap.get("ADDR_ZIP").trim();
					if(addrZip.length() > 5) {
						addrZip.substring(0, 5);
					}
				}
				
				regCntSEA200Map.put("ZIP", addrZip);			// 우편번호
				regCntSEA200Map.put("PST_ADDR", grpMap.get("STDNT_ADDR"));	// 우편주소
				regCntSEA200Map.put("DADDR", grpMap.get("STDNT_DTL_ADDR"));	// 상세주소
				
				iGradeSeCd = grpMap.get("LAST_SCHL_GRADE_NM").trim().indexOf("미취학");
				LOGGER.debug("iGradeSeCd111	 ::::::::"          + iGradeSeCd);				
				if(iGradeSeCd > -1) {
					sGradeSeCd = "01";
				}
				iGradeSeCd = grpMap.get("LAST_SCHL_GRADE_NM").trim().indexOf("초등학교");
				LOGGER.debug("iGradeSeCd222	 ::::::::"          + iGradeSeCd);
				if(iGradeSeCd > -1) {
					sGradeSeCd = "02";
				}
				iGradeSeCd = grpMap.get("LAST_SCHL_GRADE_NM").trim().indexOf("중학교");
				LOGGER.debug("iGradeSeCd333	 ::::::::"          + iGradeSeCd);
				if(iGradeSeCd > -1) {
					sGradeSeCd = "03";
				}
				iGradeSeCd = grpMap.get("LAST_SCHL_GRADE_NM").trim().indexOf("고등학교");
				...
				LOGGER.debug("학력구분코드222	 ::::::::"          + grpMap.get("LAST_SCHL_GRADE_NM").indexOf("고등학교"));
				...
				regCntSEA200Map.put("RCPT_PIC_NO", sEnfsnNo);						// 접수담당자번호
				regCntSEA200Map.put("RCPT_INST_NO", sEnfsnInstNo);					// 접수기관번호
				...
				if("동의".equals(grpMap.get("STTY_AGT_WRTCNS_SBMSN_YN"))) {
					regCntSEA200Map.put("PRTCR_AGRE_YN", "Y");						// 보호자동의여부	
				}else {
					regCntSEA200Map.put("PRTCR_AGRE_YN", "N");						// 보호자동의여부
				}
				if("동의".equals(grpMap.get("STDNT_WRTCNS_SBMSN_YN"))) {
				...
				regCntSEA200Map.put("PRTCR_TELNO", grpMap.get("STTY_AGT_CTTPC_TELNO")); // 보호자전화번호
				regCntSEA200Map.put("PRTCR_MBL_TELNO_ENCPT", "");					// 보호자휴대전화번호암호화
				
//				regCntSEA200Map.put("PRTCR_EML_ADDR_ENCPT", mask.addressMasking( scpDb.scpEncB64( grpMap.get("STTY_AGT_EML_ADDR_ENCPT")))); // 보호자이메일주소암호화
				regCntSEA200Map.put("PRTCR_EML_ADDR_ENCPT", grpMap.get("STTY_AGT_EML_ADDR_ENCPT")); // 보호자이메일주소암호화
				...
				regCntSEA200Map.put("FRST_RGTR_ID", "BATCH");
				regCntSEA200Map.put("LAST_MDFR_ID", "BATCH");
				
				// 2022.08.17 대상자정보,학업중단,학력상태 insert 추가
				regCntSEA200 = eduLinkMapper.insertTrprInfo(regCntSEA200Map); // SEA200(대상자정보) insert				
				if (regCntSEA200 > 0) {
					// 데이터변경구분코드
					regCntSEA200Map.put("DATAA_CHG_SE_CD", "I");					
					eduLinkMapper.insertTrprInfoHstr(regCntSEA200Map); // SEA201(대상자정보이력) insert					
				}
				
				if(grpMap.get("SCHULW_DSCNTC_YMD").replace(".", "") != null && !"".equals(grpMap.get("SCHULW_DSCNTC_YMD").replace(".", "")) && !"null".equals(grpMap.get("SCHULW_DSCNTC_YMD").replace(".", ""))) {
					...
					regCntSEA240Map.put("SCHULW_DSCNTC_YMD", grpMap.get("SCHULW_DSCNTC_YMD").replace(".", "")); // 학업중단일자
					...
					iGradeSeCd1 = grpMap.get("LAST_SCHL_GRADE_NM").trim().indexOf("1");
					if(iGradeSeCd1 > -1) {
						sGradeSeCd1 = "01";
					}
					iGradeSeCd1 = grpMap.get("LAST_SCHL_GRADE_NM").trim().indexOf("2");
					if(iGradeSeCd1 > -1) {
						sGradeSeCd1 = "02";
					}
					iGradeSeCd1 = grpMap.get("LAST_SCHL_GRADE_NM").trim().indexOf("3");
					if(iGradeSeCd1 > -1) {
						sGradeSeCd1 = "03";
					}
					iGradeSeCd1 = grpMap.get("LAST_SCHL_GRADE_NM").trim().indexOf("4");
					if(iGradeSeCd1 > -1) {
						sGradeSeCd1 = "04";
					}	
					iGradeSeCd1 = grpMap.get("LAST_SCHL_GRADE_NM").trim().indexOf("5");
					if(iGradeSeCd1 > -1) {
						sGradeSeCd1 = "05";
					}	
					iGradeSeCd1 = grpMap.get("LAST_SCHL_GRADE_NM").trim().indexOf("6");
					if(iGradeSeCd1 > -1) {
						sGradeSeCd1 = "06";
					}	
					
					LOGGER.debug("학년구분코드111	 ::::::::"          + sGradeSeCd1);
					LOGGER.debug("학년구분코드222	 ::::::::"          + grpMap.get("LAST_SCHL_GRADE_NM").indexOf("3"));
					...
					regCntSEA240Map.put("RM_CN", grpMap.get("SCHULW_DSCNTC_CS_CN") + " / " + grpMap.get("HNCFRT_HPE_COSE_NM")); // 비고내용
					regCntSEA240Map.put("DEL_YN", "N");									// 삭제여부
					regCntSEA240Map.put("FRST_RGTR_ID", "BATCH");
					regCntSEA240Map.put("LAST_MDFR_ID", "BATCH");
					
					regCntSEA240 = eduLinkMapper.insertSchulwDscntc(regCntSEA240Map); // SEA240(학업중단) insert				
					if (regCntSEA240 > 0) {
						// 데이터변경구분코드
						regCntSEA240Map.put("DATAA_CHG_SE_CD", "I");					
						eduLinkMapper.insertSchulwDscntcHstr(regCntSEA240Map); // SEA241(학업중단이력) insert					
					}	
				} // end if(grpMap.get("SCHULW_DSCNTC_YMD").replace(".", "") != null && !"".equals(grpMap.get("SCHULW_DSCNTC_YMD").replace(".", "")) && !"null".equals(grpMap.get("SCHULW_DSCNTC_YMD").replace(".", ""))) {
				...
				String sSchlNm[] = grpMap.get("LAST_SCHL_GRADE_NM").split(",");
				...
				regCntSEA230Map.put("FRST_RGTR_ID", "BATCH");
				regCntSEA230Map.put("LAST_MDFR_ID", "BATCH");
				
				regCntSEA230 = eduLinkMapper.insertAcbgStts(regCntSEA230Map); // SEA230(학력상태) insert
				if (regCntSEA230 > 0) {
					// 데이터변경구분코드
					regCntSEA230Map.put("DATAA_CHG_SE_CD", "I");					
					eduLinkMapper.insertAcbgSttsHstr(regCntSEA230Map); // SEA231(학력상태이력) insert					
				}
				
			} // end if("01".equals(sUldRqstdoSttsSeCd) || "02".equals(sUldRqstdoSttsSeCd)) { // 01:신규등록 02:중복등록
			String sIdx = ""+i;
			grpMap.put("MNG_SN", sTime);
			grpMap.put("INDEX_SN", sIdx);
			grpMap.put("ERR_INFO_DTL_CN", sErrInfoDtlCn);
			grpMap.put("ULD_RQSTDO_STTS_SE_CD",  sUldRqstdoSttsSeCd);
			grpMap.put("ENFSN_NO", moeCode);
			grpMap.put("FRST_RGTR_ID", "BATCH");
			grpMap.put("LAST_MDFR_ID", "BATCH");
			grpMap.put("LINK_SN", esbSeq);
			eduLinkMapper.insertOfcdcGrFileUldDtl(grpMap); // AKA221 insert
			eduLinkMapper.updateEsbStatus(esbSeq);
		} // end for(int i=0; i<CAD100List.size(); i++) {	
