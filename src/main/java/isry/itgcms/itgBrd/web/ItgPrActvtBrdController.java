...
/**
 * @파일명 : ItgPrActvtBrdController.java
 * @프로그램 설명 : 통합 홍보활동 게시판
 * @작성자 : Lee SangHoon
 * @작성일 : 2023. 4. 25.
 * @수정자 : Lee SangHoon
 ...
 */
@Controller
@Api(value = "integrate Public Relation Activity Board Controller")
@RequestMapping("/itgPrActvtBrd")
public class ItgPrActvtBrdController {
	...
	@RequestMapping("/initItgPrBrd.do")
	...
	@RequestMapping("/onLoadItgPrBrd.do")
...
//		List<Map<String, Object>> listInstCode = itgBrdCmnService.selectInstCodeList();
		...
		// 시도 구분 코드 조회
		List<Map<String, Object>> listCtpvCode = itgBrdCmnService.selectCtpvCodeList();
				
		// 시군구 구분 코드 조회
		List<Map<String, Object>> listSggCode = itgBrdCmnService.selectSggCodeList();
		
//		dataRequest.setResponse("dsCmbInstCd", listInstCode);
		dataRequest.setResponse("dsCmbCtpvCd", listCtpvCode);
		dataRequest.setResponse("dsCmbSggCd", listSggCode);
				
		return new JSONDataView();

	}

	@RequestMapping("/listItgPrBrd.do")
	public View list(HttpServletRequest request, HttpServletResponse response, DataRequest dataRequest)
		...
		mapParam.put("UNITY_BBSCTT_TYPE_SE_CD", mapInit.getValue("strBbscttTypeSeCd"));		
		mapParam.put("UNT_TASKWK_SE_CD", mapInit.getValue("strTaskwkSeCd"));
		mapParam.put("ADMIN_YN", mapInit.getValue("adminYN"));
		...
		mapParam.put("CTPV_SE_CD", searchParam.getValue("strCtpvSeCd"));					
		mapParam.put("SGG_CD", searchParam.getValue("strSggCd"));
		mapParam.put("OGDP_INST_NO", searchParam.getValue("strInstNo"));
		mapParam.put("OGDP_INST_NM", searchParam.getValue("strInstNm"));
		
		mapParam.put("SEARCH_KEY", searchParam.getValue("strSearchKey"));
		mapParam.put("SEARCH_DATA", searchParam.getValue("strSearchData"));
		mapParam.put("PR_START_DATE", searchParam.getValue("strStartDate"));
		mapParam.put("PR_END_DATE", searchParam.getValue("strEndDate"));
		mapParam.put("USER_INST_NO", userVo.getInstNo());
	...
	@RequestMapping("/deleteItgPrBrd.do")
	...
	@RequestMapping("/dtlListItgPrBrd.do")
	...
	@RequestMapping("/dtlSaveItgPrBrd.do")
