...
@RequestMapping("/itgSysQnaBrd")
public class ItgSysQnaBrdController {
	...
	public View init(HttpServletRequest request, HttpServletResponse response, DataRequest dataRequest)
		...
		// 업무단위 카테고리 반환
		List<Map<String, Object>> listItgNtcCtgrybSeCd = itgBrdCmnService.selectSysCtgrySeCdList(mapParam);
	...
	public View onLoad(HttpServletRequest request, HttpServletResponse response, DataRequest dataRequest)
		...
		// 업무처리구분코드 콤보 조회
		dataRequest.setResponse("dsCmbTaskwkPrcsSeCd", mgmtCmmnCodeService.selectCommonCode("TASKWK_PRCS_SE_CD"));

		// 업무처리팀구분코드 콤보 조회
		dataRequest.setResponse("dsCmbTaskwkTaskfoSeCd", mgmtCmmnCodeService.selectCommonCode("TASKWK_TASKFO_SE_CD"));

		// 게시판 검색 콤보 조회
		dataRequest.setResponse("dsCmbSearchSeCd", mgmtCmmnCodeService.selectCommonCode("BULLETIN_SEARCH_BREAKDOWN"));
...
//		dataRequest.setResponse("dsCmbInstCd", listInstCode);
	...
	public View list(HttpServletRequest request, HttpServletResponse response, DataRequest dataRequest)
		...
		String strTaskwkPrcsSeCd = searchParam.getValue("strTaskwkPrcsSeCd");
		String strTaskwkTaskfoSeCd = searchParam.getValue("strTaskwkTaskfoSeCd");
		String strUntTaskwkSeCd = searchParam.getValue("strUntTaskwkSeCd");
		String ynOnlyMyReg = searchParam.getValue("ynOnlyMyReg");
		String over24hr = searchParam.getValue("over24hr");

		String[] arrTaskwkPrcsSeCd = null;
		if (strTaskwkPrcsSeCd.equals("all")) {
			arrTaskwkPrcsSeCd = "1,2,3,4".split(",");
		} else if (!strTaskwkPrcsSeCd.equals("")) {
			arrTaskwkPrcsSeCd = strTaskwkPrcsSeCd.split(",");
		}
		...
		mapParam.put("TASKWK_PRCS_SE_CD", arrTaskwkPrcsSeCd);
		mapParam.put("TASKWK_TASKFO_SE_CD", strTaskwkTaskfoSeCd);
		mapParam.put("strUNT_TASKWK_SE_CD", strUntTaskwkSeCd);
		mapParam.put("UNT_TASKWK_SE_CD", loginVO.getUntTaskwk());
		mapParam.put("ONLY_MY_REG", ynOnlyMyReg);
		mapParam.put("USER_ID", loginVO.getId());
		mapParam.put("OVER_24_HR", over24hr);
