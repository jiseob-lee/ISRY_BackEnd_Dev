...
var loc = location.href.indexOf("/MagicLine4Web") > -1 ? 
    location.href.substring(0, location.href.indexOf("/MagicLine4Web")) : location.href;
var arr = loc.split("/");
//for (var i=0; i < arr.length; i++) {
    //console.log(i, arr[i]);
//}
//alert(arr.length + arr[arr.length - 1]);
var contextPath = "";
//if (arr.length >= 4 && arr.length <= 5) {
if (arr.length >= 4) {
    //contextPath = arr[3];
    contextPath = arr[3] == "ISRY_BackEnd" ? "/ISRY_BackEnd" : "";
    //if (arr[3].length > 0) {
        //contextPath = "/" + contextPath;
    //}
}
//if (contextPath.endsWith("/login.do")) {
    //contextPath = contextPath.substring(0, contextPath.length - "/login.do".length)
//}

var mlDirPath = contextPath + "/MagicLine4Web/ML4Web/";
//console.log("mlDirPath", mlDirPath);
...
//		if(msg instanceof HTMLFormElement){
    ...
    if(typeof event.origin != "undefined" && event.origin.indexOf("127.0.0.1:64032") > -1){
       return;
    }
		$('#dscertContainer').hide();
