...
function finance_btnClick(){
	var selectedLi = $('.ML_lnb_area > ul').children('.sel').attr('class');
	var lnb_id =  selectedLi.split(' ')[0].replace('ML_cert_','');
	$('#ML_lnb_'+lnb_id).click();	
}
		...
		if(id=="pfx"){
			...
			for(var i=0; i<hddOptCnt; i++){
				...
				if(i==(obj.hddOpt.length-1)){
					str += '	<li id="'+id+'_driver_'+i+'" tabindex="'+(i+1)+'"><a href="#" class="wdh_210" onkeydown="closeSubDriverList(this);">'+obj.hddOpt[i].diskname+'</a></li>';
				}else{
			...
			for(var i=0; i<tokenOptCnt; i++){
				if(i==(obj.tokenOpt.length-1)){
					str += '	<li id="'+id+'_driver_'+i+'" ><a href="#" class="wdh_150" onkeydown="closeSubDriverList();">'+obj.tokenOpt[i].tokenname+'</a></li>';
				}else{
			...
			for(var i=0; i<phoneOptCnt; i++){
				...
				if(i==(obj.phoneOpt.length-1)){
					if((!isCsInstall || isCsUpdate) && (obj.phoneOpt[i].servicename != ML4WebDraw.TITLE_MOB_SVC_UBIKEYNX)){
						str += '	<li style="display:block;"><a href="#" onClick="installCheck(\''+mode+'\');" class="wdh_245" onkeydown="closeSubDriverList();">'+obj.phoneOpt[i].servicename+'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src="UI/images/download.png" /></a></li>';
					}else{
						str += '	<li id="'+id+'_driver_'+i+'" style="display:block;"><a href="#" class="wdh_245" onkeydown="closeSubDriverList();">'+obj.phoneOpt[i].servicename+'</a></li>';
					}
				}else{
					if((!isCsInstall || isCsUpdate) && (obj.phoneOpt[i].servicename != ML4WebDraw.TITLE_MOB_SVC_UBIKEYNX)){
			...
			for(var i=0; i<cloudOptCnt; i++){
				if(i==(obj.cloudOpt.length-1)){
					str += '	<li id="'+id+'_driver_'+i+'" ><a href="#" onkeydown="closeSubDriverList();">'+obj.cloudOpt[i].servicename+'</a></li>';
				}else{
	...
	if( selection && selectionCnt > 0 ){
		...
		/*if(magiclineUtil.isMobile(magiclineUtil.getOS())){
			...
			if(selectMedia === "web_kftc" || selectMedia === "web"){
				...
				if(selectMedia === "token"){
					magiclineUtil.stopClickEvt();
				}
				if( ML4WebApi.webConfig.useVirtualKeyboard ){
					if( ML4WebApi.webConfig.virtualKeyboardType === "RAON" ){
						if(keyboardCheck){
