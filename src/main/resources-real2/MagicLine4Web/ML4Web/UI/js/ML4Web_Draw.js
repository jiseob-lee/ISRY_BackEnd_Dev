...
//------------------------
	...
	// FINANCE
	STG_HTML_FINANCE  : "<li class='ML_storage_finance'><a href='#' id='.id_key.' stgIdx='.idx.' class='stg_01' title='금융인증서'><span></span></a></li>",
//--------------------------
 ...
 * */
	...
	loadCSS : function(){
		...
		if(ML4WebApi.webConfig.storageList_lnb == null || ML4WebApi.webConfig.storageList_lnb == "") {
			...
			for( var i = 0; i < ML4WebApi.webConfig.storageList_lnb.length; i++){
				...
				} else if (ML4WebApi.webConfig.storageList_lnb[i] === "finance_user" ){
					// 개인용 금융인증SDK
					if( ML4WebApi.webConfig.fincert_org_code != "" && ML4WebApi.webConfig.fincert_user_url != "" && ML4WebApi.webConfig.fincert_apikey != "" ){
						function getToday(){var e=new Date;return e.getFullYear()+("0"+(1+e.getMonth())).slice(-2)+("0"+e.getDate()).slice(-2)};
						
						var head = document.getElementsByTagName('head')[0];
						var script = document.createElement('script');
						script.type= 'text/javascript';
						script.id = "fincert_user";
						var finCertUserURL = ML4WebApi.getProperty('fincert_user_url') + "?dt=" + getToday();
						script.src = finCertUserURL;
						head.appendChild(script);
					}
				} else if (ML4WebApi.webConfig.storageList_lnb[i] === "finance_corp" ){
					// 사업자용 금융인증SDK
					if( ML4WebApi.webConfig.fincert_org_code != "" && ML4WebApi.webConfig.fincert_corp_url != "" && ML4WebApi.webConfig.fincert_apikey != "" ){
						function getToday(){var e=new Date;return e.getFullYear()+("0"+(1+e.getMonth())).slice(-2)+("0"+e.getDate()).slice(-2)};
						
						var head = document.getElementsByTagName('head')[0];
						var script = document.createElement('script');
						script.type= 'text/javascript';
						script.id = "fincert_corp";
						var finCertCorpURL = ML4WebApi.getProperty('fincert_corp_url') + "?dt=" + getToday();
						script.src = finCertCorpURL;
						head.appendChild(script);
					}
				}
			}
	...
	_createWindow : function() {
		...
		$("#ML_window").on('close', function (event) { 
	...
	initWebMainEvent : function() {
		...
		$('[id^="ML_lnb_"]').click(function(){
			...
			if(lnb_id == "share"){
				$("#Fcert").css('display', 'none'); //금융인증서 문구 제거
				...
				$("#Fcert").css('display', 'block'); //금융인증서 문구 생성
				...
				if(lnb_id == "finance"){
					...
					//$('#ML_lnb_share').click(); //공동인증서 탭으로 돌아가기 기능
						
				} else if(lnb_id == "finance_user") {
					magiclineController.InitFinCertUserAPI(function(){
						ML4WebApi.signFinCertUserAPI(ML4WebDraw.saveParam, function(code, jsonObj){
							if(code == 0){
								var requestStr = JSON.stringify(jsonObj);
								// - cross domain, send data
								magiclineController.sendResultMessage(requestStr);
				...
				} else if(lnb_id == "finance_corp") {
					magiclineController.InitFinCertCorpAPI(function(){
						ML4WebApi.signFinCertCorpAPI(ML4WebDraw.saveParam, function(code, jsonObj){
							if(code == 0){
								var requestStr = JSON.stringify(jsonObj);
								// - cross domain, send data
								magiclineController.sendResultMessage(requestStr);
		...
		$('[id^="stg_"]').click(function () {
			...
			//220929 금융인증서 인증서 선택창에서 클릭 이벤트 로직 추가
			}else if(stg_id == "finance"){
				magiclineController.InitFinCertAPI(function(){
					ML4WebApi.signFinCertAPI(ML4WebDraw.saveParam, function(code, jsonObj){
						if(code == 0){
							var requestStr = JSON.stringify(jsonObj);
			...
			//220713 웹접근성 코드 추가 
			var tag_a_List = $('[class^="ML_storage_"] > a');
			for(var i = 0; i<tag_a_List.length; i++){
				var defaultTitle = tag_a_List[i].title.replace(" 선택됨","");
				tag_a_List[i].title = defaultTitle;
			}

			$(this).parent().addClass("on");
			var setTitle = $(this).attr("title") + " 선택됨";
...
*/
	...
	MakeStorageListDiv : function(obj) {
		...
		if(stgArr_lnb != null && stgArr_lnb.length > 0 && obj != "mgmt"){
			...
			//왼쪽날개 draw 끝	
			stgHtml_lnb += '</ul>';
			stgHtml_lnb += '<div class="ds_wm"><span>(주)드림시큐리티</span></div>';
		...
		if(stgArr != null && stgArr.length > 0){
			...
			for(var i=0; i<stg_length; i++){
				...
				case 'unisign':
					...
					break;
				case 'finance':
					stgHtml += ML4WebDraw.STG_HTML_FINANCE.replace(".id_key.", id_key + "finance").replace(".idx.", idx);
					break;
				} // end of switch
...
//						$(rowID).attr("title", strAlert);
	...
	//		ML4WebLog.log("dataTable.rowSelect.rowKey === " +rowKey);
		...
		//220713 웹접근성 코드 추가 
		// 드라이버 선택 후 인증서 첫번째 리스트 focus
		if($('#flagInit').val() == 1 && ($('#temp').val() != '' && $('#temp').val() != 0)){
			$("#row0dataTable>td").focus();
		}
	},
	openCSInstallDialog : function(mode){
		...
		if(os_ver.indexOf("LINUX") > -1){
			$("#csContainer").load("UI/ML4Web_Cs_Linux_Install.html?random=" + magiclineUtil.makeRandomValue() * 99999, function(){
		...
		if(id == "hdd"){
			...
			for(var i=0; i<hddOptCnt; i++){
				...
				if(i==(obj.hddOpt.length-1)){
					...
					htmlStream += '	<li id="'+id+'_driver_'+i+'" ><a href="#" class="wdh_210" onkeydown="closeSubDriverList();">'+obj.hddOpt[i].diskname+'</a></li>';					
				}else{
