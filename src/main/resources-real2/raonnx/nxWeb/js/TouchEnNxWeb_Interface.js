...
var touchennxwebparams = [
	...
	touchennxwebpolicy+"",
	"sremote.exe,Seetrol_Rem.exe,KakaoTalk.exe,Stup.exe,StupHlp.exe,SZAntiRansom.exe,szbackup.exe,szdaoalert.exe,szdaobuild.exe,szdaocbm.exe,szdaocrypt.exe,szdaodir.exe,szdaoemg.exe,szdaoinfo.exe,szdaologin.exe,szdaonotice.exe,szdaopolrcv.exe,szdaoregist.exe,szdaosue.exe,szdaosvc.exe,szdaotray.exe,szdaouncert.exe,szdaousr.exe,szdaoutil64.exe,szdaoutil86.exe,szdaoviewer.exe,szdaows.exe,szdebug.exe,szinsthelper.exe,SZPrivacy.exe,SZPrivacyIsolation.exe,SZPrivacyRT.exe,SZPrivacyScan.exe,SZPrivacyWidget.exe,szreplug.exe,SZSendPrivacyLog.exe,szupdate.exe,TUC.exe",
	//document.title,
	//top.document.title
	//top.frames[0].document.title
	//frames.document.title
	//""
	location.href.indexOf("login.do") == -1 ? "청소년안전망시스템" : "청소년안전망시스템 로그인"
];
...
var touchennxwebInfo = {
	...
	"exModuleName"		: "nxweb",
	"tkInstallpage"		: "resource/install/install.html",            // 설치페이지
	//"tkMainpage"		: "http://localhost:8880/ISRY_BackEnd/",       //설치완료 후 이동페이지 
	"tkMainpage"        : "http://"+location.hostname+":"+location.port+"/"+contextPath+"/",       //설치완료 후 이동페이지 
	"tkInstalled"		: false,
	"exInstalled"		: false,
	"clInstalled"		: false,
