...
<%
	...
	String rstInfo = mscr.msgDecrypt(encPriInfo,"/app/ISRY_BackEnd.war/WEB-INF/mok/cert/youthsafePri.key","goodyouth","EUC-KR");
	...
	if (rstInfoArray.length < 3 || !resultcd.equals("00")) {
		...
		if(diffMin > min){
			...
			out.println("<p>ci : " + ci);
			out.println("<p>di : " + di);
			out.println("<p>전화번호 : " + telnum );
			out.println("<p>통신사 : " + telco );
			out.println("<p>생년월일 : " + birthday);
			out.println("<p>성별 : " + gender);
			out.println("<p>내외국인 : " + nation);
			out.println("<p>이름 : " + name);
			out.println("<p>거래요청번호 : " + reqNum);
			out.println("<p>요청시간 : "+reqdate);
		}
